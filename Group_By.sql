...
with a as(select nominee, count(*) as wins from oscar_nominees where winner=1 group by nominee order by wins desc,nominee limit 1)
select distinct n.top_genre as genre from nominee_information n inner join a on a.nominee = n.name