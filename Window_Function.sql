...
with a as(select user_id,created_at,lag(created_at) over(partition by user_id order by created_at) as pre from amazon_transactions)
select distinct user_id from a where pre is not null and datediff(created_at,pre)<=7;